...
def compare_path(obs_map, rrt_path, dijkstra_path):
    ...
    for i in range(len(rrt_path)-1):
        cv2.line(obs_map, rrt_path[i], rrt_path[i+1], (255,0,0), thickness=2, lineType=8)
    print("3")
    for i in range(len(dijkstra_path)-1):
        cv2.line(obs_map, dijkstra_path[i], dijkstra_path[i+1], (0,255,0), thickness=2, lineType=8)
    print("4")
    match_points = []
    for d_path in D_PATH:
        for r_path in R_PATH:
            if d_path == r_path:
                match_points.append(d_path)

                cv2.circle(obs_map, (d_path[0], obs_map.shape[0] - d_path), 2, (0,0,255), -1)
    print("Matching points: ",match_points)
    cv2.imshow("Win", obs_map)
    cv2.imwrite("outpur_matching.jpg", obs_map)
    cv2.waitKey(0)
